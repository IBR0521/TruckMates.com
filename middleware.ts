...
function applySecurityHeaders(response: NextResponse, nonce: string) {
  ...
  // Never send HSTS on local dev — Next.js listens on plain HTTP; browsers cache HSTS and then
  // retry https://localhost, which yields "invalid response".
  if (process.env.NODE_ENV === 'production') {
...
const DASHBOARD_ROUTE_FEATURES: Array<{ pattern: RegExp; feature: FeatureCategory }> = [
  ...
  { pattern: /^\/dashboard\/trailers(\/|$)/, feature: 'vehicles' },
  { pattern: /^\/dashboard\/routes(\/|$)/, feature: 'routes' },
  { pattern: /^\/dashboard\/geofencing(\/|$)/, feature: 'routes' },
  { pattern: /^\/dashboard\/loads(\/|$)/, feature: 'loads' },
  { pattern: /^\/dashboard\/edi(\/|$)/, feature: 'loads' },
  { pattern: /^\/dashboard\/hazmat(\/|$)/, feature: 'loads' },
  { pattern: /^\/dashboard\/permits(\/|$)/, feature: 'loads' },
  ...
  { pattern: /^\/dashboard\/compliance(\/|$)/, feature: 'ifta' },
  { pattern: /^\/dashboard\/payables(\/|$)/, feature: 'accounting' },
  { pattern: /^\/dashboard\/billing\/detention-candidates(\/|$)/, feature: 'accounting' },
