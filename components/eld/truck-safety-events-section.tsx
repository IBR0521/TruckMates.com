...
import { getHarshEventCountsForTruck, getHarshEventsForCompany } from "@/app/actions/eld-events"
...
export function TruckSafetyEventsSection({ truckId }: { truckId: string }) {
  ...
  useEffect(() => {
    let alive = true
    void Promise.all([
      getHarshEventCountsForTruck({ truckId }),
      getHarshEventsForCompany({ truckId, daysBack: 30, limit: 15 }),
    ]).then(([c, e]) => {
      if (!alive) return
  ...
  return (
    <FeatureLock
      ...
      title="Vehicle safety events"
      description="Professional+ includes harsh-event sync for trucks linked to ELD devices."
    >
      <Card className="border-border/70 bg-card/80 p-4 md:p-6">
        ...
        <ul className="mt-4 divide-y divide-border/50">
          {events.length === 0 ? (
            <li className="py-3 text-sm text-muted-foreground">No recent harsh events for this vehicle.</li>
          ) : (
            events.map((ev) => (
