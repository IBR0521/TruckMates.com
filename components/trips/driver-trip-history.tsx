...
import { listTripSummariesForDriver, type TripSummaryListRow } from "@/app/actions/trip-reports"
import { Route } from "lucide-react"

export function DriverTripHistorySection({ driverId }: { driverId: string }) {
  const [rows, setRows] = useState<TripSummaryListRow[] | null>(null)

  useEffect(() => {
    let alive = true
    void listTripSummariesForDriver(driverId).then((r) => {
      if (!alive) return
  ...
  return (
    ...
    >
      <Card className="border-border/70 bg-card/80 p-4 md:p-6">
        ...
        {!rows || rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">No completed trip summaries for this driver yet.</p>
        ) : (
