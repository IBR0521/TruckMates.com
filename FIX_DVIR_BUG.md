# Fix DVIR Audit Report Bug

## Issue
Error: "column u.name does not exist"

## Cause
The SQL function `get_dvirs_for_audit` was trying to use `u.name` but the `users` table has `full_name` instead.
...
BEGIN
  ...
  SELECT 
    ...
    d.certified,
    u.full_name AS certified_by_name,  -- FIXED: Changed from u.name to u.full_name
    d.certified_date
  FROM public.dvir d
...
4. Click "Run"
5. The function will be updated and the error should be resolved
