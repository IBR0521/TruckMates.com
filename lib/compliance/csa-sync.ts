// CSA score sync workers. NOT a "use server" module (F9): these run on the service-role admin client
// and take a companyId, so exposing them as server actions made them client-callable cross-tenant
// endpoints (and `syncAllCompaniesCSAScores` a full-fleet cost/DoS trigger). They are imported only by
// the sync-csa-scores cron. The client-facing reader stays in app/actions/csa-scores.ts.
...
async function createThresholdAlerts(companyId: string, month: string, scores: Record<CSACategoryKey, number | null>) {
  ...
  for (const [key, value] of overWarning) {
    ...
    runAgentEvaluation({
      ...
      Sentry.captureException(err, { tags: { source: "agent", file: "csa-sync.ts" } })
    })
...
/**
 * Sync one company's CSA scores from the FMCSA snapshot. Internal worker — the caller (cron or an
 * authenticated server action) is responsible for supplying a companyId it is authorized to act on.
 */
