-- Extended Loads Schema to match TruckLogics functionality
-- This adds all fields needed for comprehensive load management

-- Add fields to loads table for TruckLogics-style load management
ALTER TABLE public.loads
  -- Load Type
  ADD COLUMN IF NOT EXISTS load_type TEXT DEFAULT 'ftl', -- 'ftl' or 'ltl'
  
  -- Customer Reference (already exists via company_name and customer_reference, but add customer_id)
  ...
  ADD COLUMN IF NOT EXISTS length DECIMAL(10, 2), -- in feet
  ADD COLUMN IF NOT EXISTS width DECIMAL(10, 2), -- in feet
  ADD COLUMN IF NOT EXISTS height DECIMAL(10, 2), -- in feet
  ADD COLUMN IF NOT EXISTS temperature DECIMAL(5, 2), -- for reefer loads
  ...
  ADD COLUMN IF NOT EXISTS rate_type TEXT, -- 'per-mile', 'flat-rate', 'per-ton', 'per-hundred-weight'
...
CREATE INDEX IF NOT EXISTS idx_loads_pickup_date ON public.loads(load_date);
CREATE INDEX IF NOT EXISTS idx_loads_estimated_delivery ON public.loads(estimated_delivery);
