...
export default function DVIRPageClient({
  initialDVIRs = null,
  initialError = null,
}: {
  initialDVIRs?: unknown[] | null
  initialError?: string | null
}) {
  const router = useRouter()
  const seededDvirs = initialDVIRs
    ? (initialDVIRs as unknown[]).map(asDvirRow).filter((dvir): dvir is DvirRow => !!dvir)
    : null
  const [dvirRecords, setDvirRecords] = useState<DvirRow[]>(() => seededDvirs || [])
  const [filteredDvirs, setFilteredDvirs] = useState<DvirRow[]>(() => seededDvirs || [])
  const [isLoading, setIsLoading] = useState(() => !initialDVIRs && !initialError)
  const [hasLoadedOnce, setHasLoadedOnce] = useState(() => !!initialDVIRs || !!initialError)
