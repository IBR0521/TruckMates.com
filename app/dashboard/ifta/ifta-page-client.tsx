...
export default function IFTAPageClient({
  initialReports = null,
  initialError = null,
}: {
  initialReports?: unknown[] | null
  ...
  const [iftaReports, setIftaReports] = useState<IftaReportRow[]>(() =>
    initialReports
      ? (initialReports as unknown[]).map(asIftaReportRow).filter((report): report is IftaReportRow => !!report)
      : [],
  )
  const [isLoading, setIsLoading] = useState(() => !initialReports && !initialError)
  const [hasLoadedOnce, setHasLoadedOnce] = useState(() => !!initialReports || !!initialError)
