...
export default function CustomersPageClient({
  ...
  initialCustomers?: unknown[] | null
  initialError?: string | null
}) {
  const router = useRouter()
  const [deleteId, setDeleteId] = useState<string | null>(null)
  const [deleteDependencies, setDeleteDependencies] = useState<DeleteDependency[]>([])
  const [customersList, setCustomersList] = useState<CustomerRow[]>(() => (initialCustomers as CustomerRow[]) || [])
  const [isLoading, setIsLoading] = useState(() => !initialCustomers && !initialError)
  const [hasLoadedOnce, setHasLoadedOnce] = useState(() => !!initialCustomers || !!initialError)
