...
export default function DocumentsPageClient({
  ...
  initialDocuments?: unknown[] | null
  initialError?: string | null
}) {
  const router = useRouter()
  const seededDocuments = initialDocuments
    ? (initialDocuments as unknown[]).map(asDocumentRow).filter((doc): doc is DocumentRow => !!doc)
    : null
  const [deleteId, setDeleteId] = useState<string | null>(null)
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set())
  const [isDeleting, setIsDeleting] = useState(false)
  const [documentsList, setDocumentsList] = useState<DocumentRow[]>(() => seededDocuments || [])
  const [filteredDocuments, setFilteredDocuments] = useState<DocumentRow[]>(() => seededDocuments || [])
  const [isLoading, setIsLoading] = useState(() => !initialDocuments && !initialError)
  const [hasLoadedOnce, setHasLoadedOnce] = useState(() => !!initialDocuments || !!initialError)
  ...
  const handleBulkDelete = async () => {
    ...
    if (result.error) {
      ...
      toast.success(`Successfully deleted ${result.deletedCount || ids.length} document${ids.length > 1 ? 's' : ''}`)
    ...
    <div className="w-full">
      ...
      <div className="p-8">
        <div className="max-w-7xl mx-auto space-y-6">
          {/* Summary Cards - Only show when there are documents */}
          {documentsList.length > 0 && (
            <>
              ...
              {/* Expiry Alerts Banner - Only show if there are expiring/expired documents */}
              {(() => {
                const expiringCount = documentsList.filter(d => {
                  if (!d.expiry_date) return false
                  const expiry = new Date(d.expiry_date)
                  const now = new Date()
                  const daysUntilExpiry = (expiry.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
                ...
                const expiredCount = documentsList.filter(d => {
                  if (!d.expiry_date) return false
                  const expiry = new Date(d.expiry_date)
                  const now = new Date()
                  return expiry.getTime() < now.getTime()
                        ...
                        <p className="text-sm text-muted-foreground">
                          {expiredCount > 0 && (
                            <span className="text-red-400 font-medium">
                              {expiredCount} expired{expiredCount !== 1 ? "s" : ""}
                            </span>
                          )}
          ...
          {/* Filters and Bulk Actions - Only show when there are documents */}
          {documentsList.length > 0 && (
            <Card className="p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {selectedIds.size > 0 && (
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const confirmed = window.confirm(`Are you sure you want to delete ${selectedIds.size} document${selectedIds.size > 1 ? 's' : ''}? This action cannot be undone.`)
                          if (confirmed) {
                            handleBulkDelete()
                          }
                        }}
          ...
          {isLoading ? (
            ...
            <Card className="border border-border/50 p-8">
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">There are no documents in your document library at this time.</p>
              </div>
            </Card>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredDocuments.map((doc) => {
                ...
                <Card 
                  key={doc.id} 
                  className={`border p-6 hover:border-border/80 transition shadow-sm cursor-pointer ${
