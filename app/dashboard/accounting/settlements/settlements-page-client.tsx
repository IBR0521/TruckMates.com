...
export default function SettlementsPageClient({
  ...
  initialSettlements?: unknown[] | null
  ...
  const [settlements, setSettlements] = useState<SettlementRow[]>(() =>
    initialSettlements
      ? (initialSettlements as unknown[]).map(asSettlementRow).filter((settlement): settlement is SettlementRow => !!settlement)
      : [],
  )
  const [drivers, setDrivers] = useState<DriverOption[]>(() =>
    initialDrivers
      ? (initialDrivers as unknown[]).map(asDriverOption).filter((driver): driver is DriverOption => !!driver)
      : [],
  )
  const [isLoading, setIsLoading] = useState(() => !initialSettlements && !initialError)
  const [hasLoadedOnce, setHasLoadedOnce] = useState(() => !!initialSettlements || !!initialError)
