...
export default function InvoicesPageClient({
  ...
  initialInvoices?: unknown[] | null
  initialError?: string | null
}) {
  const router = useRouter()
  const [invoices, setInvoices] = useState<InvoiceRow[]>(() =>
    initialInvoices
      ? (initialInvoices as unknown[]).map(asInvoiceRow).filter((invoice): invoice is InvoiceRow => !!invoice)
      : [],
  )
  const [isLoading, setIsLoading] = useState(() => !initialInvoices && !initialError)
  const [hasLoadedOnce, setHasLoadedOnce] = useState(() => !!initialInvoices || !!initialError)
