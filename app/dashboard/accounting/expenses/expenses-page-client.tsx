...
export default function ExpensesPageClient({
  ...
  initialExpenses?: unknown[] | null
  initialError?: string | null
}) {
  const router = useRouter()
  const seededExpenses = initialExpenses
    ? (initialExpenses as unknown[]).map(asExpenseRow).filter((row): row is ExpenseRow => !!row)
    : null
  const [deleteId, setDeleteId] = useState<string | null>(null)
  const [expensesList, setExpensesList] = useState<ExpenseRow[]>(() => seededExpenses || [])
  const [filteredExpenses, setFilteredExpenses] = useState<ExpenseRow[]>(() => seededExpenses || [])
  const [isLoading, setIsLoading] = useState(() => !initialExpenses && !initialError)
  const [hasLoadedOnce, setHasLoadedOnce] = useState(() => !!initialExpenses || !!initialError)
