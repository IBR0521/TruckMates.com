...
export default function MaintenancePageClient({
  ...
  initialMaintenance?: unknown[] | null
  ...
  const seededMaintenance = initialMaintenance
    ? (initialMaintenance as unknown[]).map(asMaintenanceRow).filter((record): record is MaintenanceRow => !!record)
    : null
  const [maintenanceRecords, setMaintenanceRecords] = useState<MaintenanceRow[]>(() => seededMaintenance || [])
  const [filteredMaintenance, setFilteredMaintenance] = useState<MaintenanceRow[]>(() => seededMaintenance || [])
  const [isLoading, setIsLoading] = useState(() => !initialMaintenance && !initialError)
  const [hasLoadedOnce, setHasLoadedOnce] = useState(() => !!initialMaintenance || !!initialError)
