...
export default function TrucksPageClient({
  initialTrucks = null,
  initialError = null,
}: {
  ...
  const [trucksList, setTrucksList] = useState<TruckRow[]>(() => (initialTrucks as TruckRow[]) || [])
  const [filteredTrucks, setFilteredTrucks] = useState<TruckRow[]>(() => (initialTrucks as TruckRow[]) || [])
  const [isLoading, setIsLoading] = useState(() => !initialTrucks && !initialError)
  const [hasLoadedOnce, setHasLoadedOnce] = useState(() => !!initialTrucks || !!initialError)
