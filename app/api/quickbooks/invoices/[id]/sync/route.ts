...
type QuickBooksConnection = Awaited<ReturnType<typeof getQuickBooksConnection>>
...
async function getOrCreateIncomeAccountId(conn: QuickBooksConnection, mapping: QuickBooksMapping) {
...
async function getOrCreateServiceItem(conn: QuickBooksConnection, mapping: QuickBooksMapping) {
  // Prefer explicit mapping if configured
...
async function getOrCreateCustomer(conn: QuickBooksConnection, displayName: string) {
...
export async function POST(request: NextRequest, ctxRoute: { params: Promise<{ id: string }> }) {
  try {
    ...
    const customerId = await getOrCreateCustomer(conn, customerName)
    const itemId = await getOrCreateServiceItem(conn, mapping)
    ...
    const created = await qbFetch(conn, "/invoice", { method: "POST", body: JSON.stringify(payload) })
