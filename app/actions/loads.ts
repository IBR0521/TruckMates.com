...
export async function createLoad(formData: {
  ...
  if (!routeId && settings.auto_create_route && formData.origin && formData.destination && formData.destination !== "Multiple Locations") {
    ...
    if (existingRoutes && existingRoutes.length > 0) {
      ...
      const matchingRoute = existingRoutes.find((route: { id: string; origin: string | null; destination: string | null }) => {
...
export async function updateLoad(
  ...
  const contents = asString(updateData.contents)
  if (contents) updateData.contents = sanitizeString(contents, 500)
  const shipperEmail = asString(updateData.shipper_contact_email)
  if (shipperEmail) updateData.shipper_contact_email = sanitizeEmail(shipperEmail)
  const shipperPhone = asString(updateData.shipper_contact_phone)
  if (shipperPhone) updateData.shipper_contact_phone = sanitizePhone(shipperPhone)
  const consigneeEmail = asString(updateData.consignee_contact_email)
  if (consigneeEmail) updateData.consignee_contact_email = sanitizeEmail(consigneeEmail)
  const consigneePhone = asString(updateData.consignee_contact_phone)
  if (consigneePhone) updateData.consignee_contact_phone = sanitizePhone(consigneePhone)
  const notes = asString(updateData.notes)
  if (notes) updateData.notes = sanitizeString(notes, 2000)
  const internalNotes = asString(updateData.internal_notes)
  if (internalNotes) updateData.internal_notes = sanitizeString(internalNotes, 2000)
  const specialInstructions = asString(updateData.special_instructions)
  if (specialInstructions) updateData.special_instructions = sanitizeString(specialInstructions, 1000)
  const pickupInstructions = asString(updateData.pickup_instructions)
  if (pickupInstructions) updateData.pickup_instructions = sanitizeString(pickupInstructions, 1000)
  const deliveryInstructions = asString(updateData.delivery_instructions)
  if (deliveryInstructions) updateData.delivery_instructions = sanitizeString(deliveryInstructions, 1000)
  const nmfcCode = asString(updateData.nmfc_code)
  if (nmfcCode) updateData.nmfc_code = sanitizeString(nmfcCode, 30)
  const freightClass = asString(updateData.freight_class)
  if (freightClass) updateData.freight_class = sanitizeString(freightClass, 20)
  const permitReason = asString(updateData.permit_requirement_reason)
  if (permitReason) updateData.permit_requirement_reason = sanitizeString(permitReason, 500)
  const unNumber = asString(updateData.un_number)
  if (unNumber) updateData.un_number = sanitizeString(unNumber, 20)
  const hazardClass = asString(updateData.hazard_class)
  if (hazardClass) updateData.hazard_class = sanitizeString(hazardClass, 20)
  const packingGroup = asString(updateData.packing_group)
  if (packingGroup) updateData.packing_group = sanitizeString(packingGroup, 20)
  const properShippingName = asString(updateData.proper_shipping_name)
  if (properShippingName) updateData.proper_shipping_name = sanitizeString(properShippingName, 200)
  const emergencyContact = asString(updateData.emergency_contact)
  if (emergencyContact) updateData.emergency_contact = sanitizeString(emergencyContact, 120)
  const portalRequestMessage = asString(updateData.portal_request_message)
  if (portalRequestMessage) updateData.portal_request_message = sanitizeString(portalRequestMessage, 1000)
  const requestedEquipmentType = asString(updateData.requested_equipment_type)
  if (requestedEquipmentType) updateData.requested_equipment_type = sanitizeString(requestedEquipmentType, 60)
  ...
  if (data && !wasDelivered && willBeDelivered) {
    try {
      ...
      const invoiceResult = await maybeAutoInvoiceOnDelivery(id, ctx.companyId)
      if (invoiceResult.error) {
        Sentry.captureException(new Error(invoiceResult.error))
      } else if (
        invoiceResult.data &&
        !invoiceResult.data.skipped &&
        !invoiceResult.data.alreadyExists &&
        invoiceResult.data.invoiceId &&
        invoiceResult.data.invoiceNumber
      ) {
        const mutableData = data as Record<string, unknown> & {
          autoGeneratedInvoice?: { id: string; invoice_number: string }
        }
        mutableData.autoGeneratedInvoice = {
          id: invoiceResult.data.invoiceId,
          invoice_number: invoiceResult.data.invoiceNumber,
        }
      }
    } catch (error) {
      Sentry.captureException(error)
...
export async function bulkDeleteLoads(ids: string[]) {
  ...
  if (loadsToDelete) {
    const inTransitLoads = loadsToDelete.filter((l: { id: string; shipment_number: string; status: string; [key: string]: unknown }) => l.status === "in_transit")
    if (inTransitLoads.length > 0) {
      const shipmentNumbers = inTransitLoads.map((l: { id: string; shipment_number: string; status: string; [key: string]: unknown }) => l.shipment_number).join(", ")
      return { 
    ...
    const blockedLoads = loadsToDelete.filter((l: { id: string; shipment_number: string; status: string; [key: string]: unknown }) => !allowedStatuses.includes(l.status))
    if (blockedLoads.length > 0) {
      const shipmentNumbers = blockedLoads.map((l: { id: string; shipment_number: string; status: string; [key: string]: unknown }) => l.shipment_number).join(", ")
      return { 
...
export async function getLoadSuggestions(origin?: string, destination?: string) {
  ...
  const suggestions: {
    ...
    lastUsedCustomer: unknown
    similarLoads: unknown[]
  } = {
  ...
  if (origin && destination) {
    ...
    if (recentLoads && recentLoads.length > 0) {
      ...
      const driverCounts: Record<string, number> = {}
      const truckCounts: Record<string, number> = {}
      recentLoads.forEach((load: { driver_id: string | null; truck_id: string | null; customer_id: string | null; [key: string]: unknown }) => {
        if (load.driver_id) {
          driverCounts[load.driver_id] = (driverCounts[load.driver_id] || 0) + 1
        }
        if (load.truck_id) {
